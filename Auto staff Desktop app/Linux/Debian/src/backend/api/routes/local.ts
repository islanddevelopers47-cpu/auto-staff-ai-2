...
import { checkBinary, checkBinaries, installBinary, hasHomebrew } from "../../local/dependency-checker.js";
...
export function createLocalRouter(): Router {
  ...
  router.get("/local/fs/list", (req, res) => {
    const dirPath = ((req.query.path as string) || "~").replace(/^~/, process.env.HOME || "");
  ...
  router.get("/local/fs/read", (req, res) => {
    const filePath = ((req.query.path as string) || "").replace(/^~/, process.env.HOME || "");
  ...
  router.post("/local/fs/move", (req, res) => {
    ...
    try {
      const result = localFs.moveItem(
        from.replace(/^~/, process.env.HOME || ""),
        to.replace(/^~/, process.env.HOME || "")
      );
      res.json(result);
    } catch (err: any) {
      res.status(400).json({ error: err.message });
  ...
  router.get("/local/deps/homebrew", (_req, res) => {
    res.json({ installed: hasHomebrew() });
  });
