...
class ProcessManager extends EventEmitter {
  ...
  start(
    ...
    const shellPath = options.shell || "/bin/bash";
    ...
    child.stdout?.on("data", (chunk: Buffer) => {
      ...
      for (const line of lines) {
        if (info.outputLines.length >= entry.maxOutputLines) {
          info.outputLines.shift();
        }
    ...
    child.stderr?.on("data", (chunk: Buffer) => {
      const lines = chunk.toString("utf-8").split("\n").filter(Boolean);
      for (const line of lines) {
        if (info.errorLines.length >= entry.maxOutputLines) {
          info.errorLines.shift();
        }
  ...
  /**
   * Get output from a process (last N lines).
   */
  getOutput(id: string, tail = 50): { stdout: string[]; stderr: string[] } {
    ...
    return {
      stdout: entry.info.outputLines.slice(-tail),
      stderr: entry.info.errorLines.slice(-tail),
    };
  }
  ...
  /**
   * Remove a completed/stopped process from the list.
   */
  ...
  /**
   * Stop all processes. Used during app shutdown.
   */
  stopAll(): void {
    for (const [id, entry] of this.processes) {
      if (entry.info.status === "running") {
