...
// Safety: restrict file operations to safe directories (Windows)
const ALLOWED_ROOTS = [
  os.homedir(),
  os.tmpdir(),
];
...
function isPathAllowed(targetPath: string): boolean {
  ...
  return ALLOWED_ROOTS.some((root) => resolved.startsWith(root.toLowerCase()));
}
...
export function readFile(filePath: string, maxBytes = 512_000): FileContent {
  ...
  const binaryExts = [".png", ".jpg", ".jpeg", ".gif", ".bmp", ".ico", ".pdf", ".zip", ".tar", ".gz", ".exe", ".msi", ".bin", ".dll", ".sys", ".o", ".a", ".7z", ".rar"];
  if (binaryExts.includes(ext)) {
    return { path: resolved, content: `[Binary file: ${stat.size} bytes, type: ${ext}]`, size: stat.size, encoding: "binary" };
  }

  if (stat.size > maxBytes) {
    const partial = fs.readFileSync(resolved, { encoding: "utf-8", flag: "r" }).slice(0, maxBytes);
    return { path: resolved, content: partial + `\n\n[Truncated: file is ${stat.size} bytes, showing first ${maxBytes}]`, size: stat.size, encoding: "utf-8" };
  }

  return { path: resolved, content: fs.readFileSync(resolved, "utf-8"), size: stat.size, encoding: "utf-8" };
}

export function writeFile(filePath: string, content: string): { path: string; size: number } {
  const resolved = assertPath(filePath);
  const dir = path.dirname(resolved);
  if (!fs.existsSync(dir)) { fs.mkdirSync(dir, { recursive: true }); log.info(`Created directory: ${dir}`); }
...
export function deleteItem(targetPath: string, recursive = false): { path: string; deleted: boolean } {
  ...
  const stat = fs.statSync(resolved);
  if (stat.isDirectory()) {
    if (recursive) { fs.rmSync(resolved, { recursive: true, force: true }); }
    else { fs.rmdirSync(resolved); }
  ...
  function walk(dir: string, depth: number) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory() && !entry.name.startsWith("node_modules")) walk(full, depth + 1);
      }
